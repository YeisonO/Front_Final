function autoInicioRelacionCliente(){
    ...
    $.ajax({
        url:"http://129.151.116.51:8080/api/Client/all",
        type:"GET",
        datatype:"JSON",
...
function autoInicioMachine(){
    ...
    $.ajax({
        url:"http://129.151.116.51:8080/api/Machine/all",
        type:"GET",
        datatype:"JSON",
        success:function(respuesta){
...
function agregarReservation() {
    ...
    if($("#startDate").val().length == 0 || $("#devolutionDate").val().length == 0 || $("#status").val().length == 0){
        ...
        $.ajax({
            ...
            url:"http://129.151.116.51:8080/api/Reservation/save",
...
function listarReservation(){
    $.ajax({
        url:"http://129.151.116.51:8080/api/Reservation/all",
...
function pintarRespuestaReservation(response){
    ...
    myTable+="<tr>";
        ...
        myTable+="<td>"+response[i].machine.name+"</td>";
    ...
    $.ajax(
        {
            dataType: 'json',
            data: dataToSend,
            url:"http://129.151.116.51:8080/api/Reservation/"+idElemento,
...
function cargarDatosReservation(id) {
    $.ajax({
        dataType: 'json',
        url:"http://129.151.116.51:8080/api/Reservation/"+id,
...
function actualizarReservation(idElemento) {
    ...
    if($("#startDate").val().length == 0 || $("#devolutionDate").val().length == 0 || $("#status").val().length == 0){
        ...
        $.ajax({
            ...
            data: dataToSend,
            contentType: "application/JSON",
            url:"http://129.151.116.51:8080/api/Reservation/update",
